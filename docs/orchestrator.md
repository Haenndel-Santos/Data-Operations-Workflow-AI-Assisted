...
boundary. The `src/data_ops_lab/cli_commands/` package now owns 21 parser
declarations: seven dataset-benchmark commands, eight semantic catalog,
approval, adapter, translation, and offline evaluation commands, and six query
planning, execution, presentation, narration, and session commands.
`build_parser` invokes all three registrars in the original command order,
while `data_ops_lab.cli:main` retains execution and result formatting. This is
static registration organization, not module discovery or dynamic dispatch.
