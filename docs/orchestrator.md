...
boundary. The `src/data_ops_lab/cli_commands/` package now owns 15 parser
declarations: seven dataset-benchmark commands and eight semantic catalog,
approval, adapter, translation, and offline evaluation commands.
`build_parser` invokes both registrars in the original command order, while
