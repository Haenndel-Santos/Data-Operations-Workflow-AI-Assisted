...
6. **AI-assisted analytics interface:** Stage 5A safe structured query planning, Stage 5B controlled local DuckDB execution, Stage 5C semantic governance, Stage 5D deterministic/provider-neutral translation evaluation, and a synthetic Stage 5E exact-answer harness are implemented. A concrete approved semantic registry, live model-provider integration, approved dataset-backed benchmark packs, dataset-backed expected answers, and result narration remain pending. Benchmark storage and restricted Northwind/Pubs conversion are implemented; the official AdventureWorks 2025 backup is locally restored, integrity-checked, and read-only pending export and review.
