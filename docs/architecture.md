...
| Governed cleaning engine (`governed_cleaning_engine.py`) | Opt-in propose/authorize/apply route over local Parquet: verified source hash, deterministic profiling, exact review and policy authorization, ordered hash-bound application plan, atomic apply with lineage | Never called by `run_workflow`; publishes to new directories only; re-verifies every authority and the plan against the current source before touching staging; failure before publish promotes nothing; no model or network. See [Governed Cleaning Engine](governed-cleaning-engine.md). |
